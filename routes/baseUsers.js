...
//a middlware (functionality) that will get executed for every request issued to this app.This checks the header 'Content-Type'.
app.use(function(req, res, next) {
  req.checkHeader = function() {
    var header1 = req.get('Content-Type'),
    header2 = req.get('Authorization');

    if(header1 == "text/plain;charset=UTF-8")
      return res.status(406).json({message:"Header 'Content-Type' is missing in request header."});
    else if(header1 != "application/json")
      return res.status(406).json({message:"Check the Content-Type header value. Only JSON format is supported."});
    else if(header2 == null)
      return res.status(406).json({message:"Header 'Authorization' is missing in request header."});
    else
      return false;
    }
    return next();
